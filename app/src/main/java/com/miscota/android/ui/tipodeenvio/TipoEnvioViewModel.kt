...
class TipoEnvioViewModel(
    ...
    fun setQuantityRef(
        ...
        try {
            ...
            println("Carriers CartViewModel line 143 costSD ${_costSd.value}")
            println("Carriers CartViewModel line 144 costECO ${_costEcommerce.value}")
            println("Carriers CartViewModel line 145 authStore.costSD ${authStore.getCarriersSd()}")
            println("Carriers CartViewModel line 146 authStore.costECO ${authStore.getCarriersEco()}")
            setCarriersSd(_costSd.value.toString())
            setCarriersEco(_costEcommerce.value.toString())

            val carrriers = (_costSd.value!! + _costEcommerce.value!!)
            setCarriers(carrriers.toString())**/
        } catch (e: Exception) {
            _messageEvent.value = Event(e.message)
        }
        loadCart()
    ...
    fun checkout(clientType: Boolean, itens: List<CartUiModel.ItemListCheckout>) {
        var idguess = "000"
        var idToLong = idguess.toLong()
        //userGuess: UserModel((userId=0098,name="invitado", email="fernanda@buddy.eu",sex="0",birthdate="",phone="66589098")
        try {
            viewModelScope.launch {
                val result = runCatching {
                    checkoutRepository.fetchCheckout(
                        ...
                        newsLetter = true,
                        deliveryDate = authStore.getcurrentTimeDelivered()?:"2021-06-01",
                        deliveryRange = "T1SD",
                        items = itens,
                        shippingAddress = "",
                        customAddress = true,
                        digitalDelivery = "",
                        ecommerceShippingPrice = _costDeliver.value?.toDouble()!!
                    )
                }
                if (result != null) {
                    println("result!=null")
    ...
    fun carriersCheckoutCostSd(clientType: Boolean): Double {
        println(" START CarriersSD CartViewModel line 344")
        val items: List<CartUiModel.ItemListCheckout> = loadCartToList()
        var costSd = 0.0
        try {
            viewModelScope.launch {
            ...
            println(" END CarriersSD CartViewModel line 377")
        } catch (e: Exception) {
            println("${e.message}  -Error-  ${e.printStackTrace()}")
        }
        return costSd
    ...
    fun carriersCheckoutCostEco(clientType: Boolean): Double {
        println(" START CarriersECO CartViewModel line 385")
        ...
        try {
            ...
            println(" END CarriersECO CartViewModel line 419")
        } catch (e: Exception) {
            println("${e.message}  -Error-  ${e.printStackTrace()}")
        }
        return costEco
    ...
    fun carriersCheckoutTotal(clientType: Boolean, itens: List<CartUiModel.ItemListCheckout>): Double {
        println(" START CarriersTotal CartViewModel line 427")
        var costTotal = 0.0
        try {
            viewModelScope.launch {
                val result = runCatching {
            ...
            println(" END CarriersTotal CartViewModel line 461")
        } catch (e: Exception) {
            println("${e.message}  -Error-  ${e.printStackTrace()}")
        }
        return costTotal
