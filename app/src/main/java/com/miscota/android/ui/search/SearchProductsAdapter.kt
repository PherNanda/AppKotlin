...
class SearchProductsAdapter(
...
) : ListAdapter<CategoryUiModel.Product, SearchProductsAdapter.ProductViewHolder>(
    ListDiffUtil
) {
    ...
    class ProductViewHolder(
        ...
        fun bind(item: CategoryUiModel.Product) {
            uiModel = item
            binding.priceText.text = "${item.productPrice.split("€").firstOrNull().toString()} €"

            println("binding.priceText.text ${binding.priceText.text}")
